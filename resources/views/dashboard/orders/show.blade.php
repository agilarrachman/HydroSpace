...
<body>
            ...
            <div class="layout-page">
                ...
                <nav
                    ...
                    <div class="navbar-nav-right d-flex align-items-center justify-content-between" id="navbar-collapse">
                        <h5 class="mb-0">Daftar {{ $active }} Kustomer</h5>
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <a href="/dashboard/orders" class="btn btn-primary mb-4">
                        ...
                        <div class="card p-4">
                            ...
                            <div class="mb-5 d-flex flex-column flex-md-row justify-content-between align-items-end">
                                <div class="order col-lg-5 col-12">
                                    ...
                                    @endif
                                    <h5 class="mb-1 fw-bolder mt-3">ID Pesanan: {{ $order->id }}</h5>
                                    <p class="m-0"><b>Waktu Pemesanan: </b>{{ $order->created_at->format('d F Y, H:i') }}</p>
                                    <p class="m-0"><b>Metode Pembayaran: </b><span style="text-transform: uppercase;">{{ $order->payment_method }}</span></p>
                                </div>

                                <div class="status col-lg-7 col-12 mt-3 mt-lg-0">
                                    <form action="{{ route('dashboard.orders.updateStatus') }}" method="POST">
                                        @method('put')
                                        @csrf
                                        <div class="col-lg-11 d-flex justify-content-between align-items-start mb-2">
                                            <h5 class="text-center text-md-start mb-3 px-md-5">Status Pesanan</h5>
                                            <p class="pe-2" id="update-status" style="cursor: pointer;">Update Status</p>
                                            <button type="submit" class="pe-2 py-0  d-none" id="submit-status" onclick="return confirm('Apakah kamu yakin akan mengubah status pesanan ini?')">Simpan</button>
                                        </div>
                                        <div class="status-container">
                                            <!-- Fase 1: Dikemas -->
                                            <div class="status-item">
                                                <div class="circle {{ $order->status == 'Dikemas' || $order->status == 'Diantar' || $order->status == 'Selesai' ? 'active' : '' }}">
                                            ...
                                            <!-- Fase 2: Diantar -->
                                            <div class="status-item">
                                                <div class="circle {{ $order->status == 'Diantar' || $order->status == 'Selesai' ? 'active' : '' }}">
                                                    <!-- SVG Icon untuk Diantar -->
                                                ...
                                                <div class="status-text">Diantar</div>
                                            </div>

                                            <!-- Fase 3: Selesai -->
                                            <div class="status-item">
                                                <div class="circle {{ $order->status == 'Selesai' ? 'active' : '' }}">
                                        ...
                                        <div class="radio-status mb-3 d-none col-lg-11 ps-md-5">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status" id="dikemas" value="Dikemas" {{ $order->status == 'Dikemas' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="dikemas">Dikemas</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status" id="diantar" value="Diantar" {{ $order->status == 'Diantar' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="diantar">Diantar</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status" id="selesai" value="Selesai" {{ $order->status == 'Selesai' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="selesai">Selesai</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status" id="batal" value="Dibatalkan" {{ $order->status == 'Dibatalkan' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="batal">Batalkan</label>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="product d-flex gap-3">
                                <div class="image-product d-flex">
                                    <img src="{{ asset('storage/' . $orderItem->product->picture1) }}" alt="{{ $orderItem->product->name }}" class="h-100 object-fit-cover m-auto p-0" style="background-color: transparent; border: none;">
                                </div>
                                <div class="d-info d-flex flex-grow-1 justify-content-between">
                                    ...
                                    @if ($orderItem->product)
                                    <span class="d-price">Rp{{ number_format($orderItem->product->price, 0, ',', '.') }}</span>
                                    @else
                                    <span class="d-price">Rp0</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="w-100 mt-3 d-flex justify-content-between gap-2">
                                ...
                                <h4 class="m-0 text-truncate fw-bold text-end">Rp{{ number_format($order->total_amount, 0, ',', '.') }}</p>
                            </div>

                            <div class="address mt-2">
                                <h5 class="mb-3"><b>Detail Informasi Pengiriman</b></h5>
                                <div class="row g-2 mb-3">
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <input
                                            ...
                                            placeholder="Masukkan phone_number" disabled />
                                    </div>
                                </div>
                                <div class="row g-3 mb-3">
                                    <div class="col-lg-6">
                                        <label for="provinsi" class="form-label">Provinsi</label>
                                        <input type="text" class="form-control" id="provinsi" name="provinsi" placeholder="Provinsi" value="{{ $provinces->firstWhere('id', $order->province)->name }}" disabled />
                                    </div>
                                    <div class="col-lg-6">
                                        <label for="kota" class="form-label">Kota</label>
                                        <input type="text" class="form-control" id="kota" name="kota" placeholder="Kota" value="{{ $cities->firstWhere('id', $order->city)->name }}" disabled />
                                    </div>
                                </div>
                                <div class="row g-3 mb-3">
                                    <div class="col-lg-6">
                                        <label for="kecamatan" class="form-label">Kecamatan</label>
                                        <input type="text" class="form-control" id="kecamatan" name="kecamatan" placeholder="Kecamatan" value="{{ $districts->firstWhere('id', $order->district)->name }}" disabled />
                                    </div>
                                    <div class="col-lg-6">
                                        <label for="kelurahan" class="form-label">Kelurahan</label>
                                        <input type="text" class="form-control" id="kelurahan" name="kelurahan" placeholder="Kelurahan" value="{{ $villages->firstWhere('id', $order->village)->name }}" disabled />
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="alamat" class="form-label">Alamat Lengkap</label>
                                    <textarea class="form-control" id="alamat" rows="3" style="min-height: 200px;" disabled>{{ $order->full_address }}</textarea>
                                </div>
                            </div>
                        </div>
                        <!--/ Striped Rows -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const updateStatusLink = document.getElementById('update-status');
            const submitStatusButton = document.getElementById('submit-status');
            const statusContainer = document.querySelector('.status-container');
            const statusRadioContainer = document.querySelector('.radio-status');

            updateStatusLink.addEventListener('click', function() {
                statusContainer.classList.add('d-none');
                updateStatusLink.classList.add('d-none');
                statusRadioContainer.classList.remove('d-none');
                submitStatusButton.classList.remove('d-none');
            });
        });
    </script>
</body>
